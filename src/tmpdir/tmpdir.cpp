...
exit_code_t create_temporary_directory() {
	exit_code_t exit_code = EXIT_SUCCESS;;
	std::string temporary_directory_name = std::tmpnam(nullptr);
	std::error_code error_code;
	if (!std::filesystem::create_directory(temporary_directory_name, error_code)) {
		fmt::print(stderr, "{}: cannot create temporary directory\n", program_name);
...
int main(int argc, char **argv) {
	...
	if (help->count > 0) {
		...
		fmt::print("Creates a temporary directory with a unique auto-generated filename.\n\n");
	...
	exit_code_t exit_code = create_temporary_directory();
