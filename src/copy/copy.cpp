...
exit_code_t copy(int verbose, const char *source, const char *destination) {
	exit_code_t exit_code = EXIT_SUCCESS;
	if(!std::filesystem::exists(source)) {
		fmt::print(stderr, "{}: cannot copy source '{}': No such file or directory\n", program_name, source);
		return EXIT_FAILURE;
	}

	if (std::filesystem::exists(destination) && !std::filesystem::is_directory(destination)) {
		fmt::print(stderr, "{}: cannot copy to destination '{}': File exists\n", program_name, destination);
		return EXIT_FAILURE;
	}
	
	if (std::filesystem::exists(destination) && std::filesystem::is_directory(destination)) {
		std::error_code error_code;
		std::filesystem::copy(source, destination / std::filesystem::path(source).filename(), std::filesystem::copy_options::recursive, error_code);
		if (error_code) {
			fmt::print(stderr, "{}: cannot copy '{}' to '{}': {}", program_name, source, destination, error_code.message());
			return EXIT_FAILURE;
		}
	...
	std::error_code error_code;
	std::filesystem::copy(source, destination, std::filesystem::copy_options::recursive, error_code);
	if (error_code) {
		fmt::print(stderr, "{}: cannot copy '{}' to '{}': {}", program_name, source, destination, error_code.message());
	...
	if (verbose > 0) {
		fmt::print("{}: copied '{}' to '{}'", program_name, source, destination);
	}
	return exit_code;
}
...
int main(int argc, char **argv) {
	...
	if (help->count > 0) {
		...
		fmt::print("Copy SOURCE to DESTINATION\n\n");
	...
	exit_code_t exit_code = copy(verbose->count, (source->filename)[0], (destination->filename)[0]);
